...
static int wiphy_verify_combinations(struct wiphy *wiphy)
{
	...
	int i, j;
        ...
        printk("mac802: start verify combinations\n");
	...
	for (i = 0; i < wiphy->n_iface_combinations; i++) {
		...
		for (j = 0; j < c->n_limits; j++) {
			...
			if (WARN_ON(types & all_iftypes)) {
                                printk("mac80211: types overlap\n");
				return -EINVAL;
                        }
			all_iftypes |= types;

                        printk("mac80211: debug tag 6\n");
			if (WARN_ON(!c->limits[j].max))
				return -EINVAL;

			/* Shouldn't list software iftypes in combinations! */
                        printk("mac80211: debug tag 7\n");
			 ...
			 * in a combination.
			 */
                        printk("mac80211: debug tag 10\n");
			if (WARN_ON((wiphy->interface_modes & types) != types)) {
                                printk("mac80211: unsupported type\n");
				return -EINVAL;
                        }
		}

                printk("mac80211: verify combinations end\n");
        ...
        printk("mac802: verify combinations return\n");
...
int wiphy_register(struct wiphy *wiphy)
{
	...
	if (WARN_ON(ifmodes != wiphy->interface_modes))
        ...
        printk("mac80211: verify combinations \n");
	...
	/* set up regulatory info */
        printk("mac80211: regulatory register\n");
