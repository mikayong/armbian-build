...
int ieee80211_register_hw(struct ieee80211_hw *hw)
{
	...
	if (!local->use_chanctx) {
		for (i = 0; i < local->hw.wiphy->n_iface_combinations; i++) {
			...
			if (comb->num_different_channels > 1) {
                            printk("mac80211: combinations num_different_channels > 1 \n");
				return -EINVAL;
                        }
		}
	} else {
	...
	/* mac80211 doesn't support more than one IBSS interface right now */
        printk("mac80211: combinations set: \n");
	...
	wiphy_lock(hw->wiphy);
        ...
        printk("mac80211: add virtual iface.\n");
 ...
 fail_wiphy_register:
        printk("mac80211: fail wiphy register.\n");
	rtnl_lock();
